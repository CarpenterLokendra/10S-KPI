...
class SoundService {
  ...
  private alarmIntervals: Set<NodeJS.Timeout> = new Set()
  ...
  registerAlarmInterval(interval: NodeJS.Timeout) {
    this.alarmIntervals.add(interval)
  }

  stopAlarm() {
    this.alarmIntervals.forEach(interval => {
