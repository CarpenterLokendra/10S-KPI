...
export const lobbyService = {
  ...
  async markReady(code: string): Promise<{ message: string; is_ready: boolean; ready_players: number; total_players: number; all_ready: boolean }> {
    const response = await axios.post(API_ENDPOINTS.LOBBY_READY(code), {})
    return response.data
  },
